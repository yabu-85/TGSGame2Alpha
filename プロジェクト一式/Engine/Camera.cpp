...
{
	_proj = XMMatrixPerspectiveFovLH(1.0f * zoom, ((FLOAT)Direct3D::screenWidth_) / (FLOAT)Direct3D::screenHeight_, 0.1f, 10000.0f);
}

//二人用プロジェクション行列計算
void Camera::SetTwoProjectionMatrix(float zoom)
{
	_proj = XMMatrixPerspectiveFovLH(1.0f * zoom, ((FLOAT)Direct3D::screenWidth_ / 2.0f) / (FLOAT)Direct3D::screenHeight_, 0.1f, 10000.0f);
}
