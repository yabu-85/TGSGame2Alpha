...
namespace {
	const float CAMERA_ROTATE_SPEED = 0.005f;	//回転速度
	const float CAMERA_CENTER = 50.0f;			//回転中心
	const float CAMERA_POS_X = 15.0f;			//カメラの遠さ
	const float CAMERA_POS_Y = 15.0f;			//カメラの高さ

}

//コンストラクタ
TitleScene::TitleScene(GameObject * parent)
	: SceneBase(parent, "TitleScene"), time_(0), stageModel_(0), cameraRotate_(0.0f), lightPos_{ XMFLOAT3(), XMFLOAT3() }
{
}
...
void TitleScene::Initialize()
{
	...
	Camera::SetFovAngleParcent(1.0f, 0);
	Camera::Update(0);
...
void TitleScene::Update()
{
	...
	//カメラの回転
	cameraRotate_ += CAMERA_ROTATE_SPEED;
	float CamX = CAMERA_CENTER + CAMERA_POS_X * std::cosf(cameraRotate_);
	float CamZ = CAMERA_CENTER + CAMERA_POS_X * std::sinf(cameraRotate_);
	Camera::SetPosition(XMFLOAT3(CamX, CAMERA_POS_Y, CamZ), 0);
