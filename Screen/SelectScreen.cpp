...
SelectScreen::SelectScreen() : Screen(), hPict_{ -1, -1 }
{
	...
	AddUI("A", UI_BUTTON, XMFLOAT2(-0.5f, 1.0f), XMFLOAT2(0.5, 0.2f), [this]() { if(!uiList_.empty()) Kill(uiList_.at(0)); });
	AddUI("B", UI_BUTTON, XMFLOAT2(0.5f, 1.0f), XMFLOAT2(0.5, 0.2f), [this]() { if(!uiList_.empty()) Kill(uiList_.at(0)); });

}

SelectScreen::~SelectScreen()
{
	Image::Release(hPict_[0]);
	Image::Release(hPict_[1]);
